    ...
    while True:
        try:
            async with websockets.connect(uri) as websocket:
                ...
                await subscribe_to_klines(websocket, symbols, id, coin_meta_pool, strategy_pool)
...
async def subscribe_to_klines(websocket, symbols, id, coin_meta_pool, strategy_pool):
    while True:
        subscribe_message = {
            "method": "SUBSCRIBE",
            "params": symbols,
        ...
        if all_symbols.get(symbol, '') != response_data['k']['T']:
            ...
            time.sleep(1)
            continue
        print(symbol, float(response_data['k']['T']), close_price)
